...
- Every real public file must have a direct download action. As of 2026-08-27, all six public products (`label`, `bleed`, `multisize-bleed`, `pdf`, `packing`, and `accounting`) have verified public client releases and release-support files. `label`, `pdf`, and `accounting` expose Windows 10/11 x64 and Windows 7 x64 clients where declared by their release manifests; activation generators, private keys, central-server material, and internal administration tools remain private and must never be published.
- `packing` is priced at `¥499 / 年`. `accounting` is priced at `¥999 / 账号 / 年`, and one account means one enterprise ledger subject (`一个账号对应一个企业账套主体`). Do not describe either product as using the four prepress tools' one-hour trial rule.
