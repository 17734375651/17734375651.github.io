...
- Every real public file must have a direct download action. As of 2026-09-03, all six public products (`label`, `bleed`, `multisize-bleed`, `pdf`, `packing`, and `accounting`) have verified pure-offline 30-day-trial client releases and release-support files. The current release set targets Windows 10/11: `bleed` is x86 and the other five are x64. Historical Windows 7 downloads remain in older GitHub Releases and are not advertised as the new trial builds. Activation generators, private keys, central-server material, and internal administration tools remain private and must never be published.
