...
function ProductsSection({ showHeading = true }) {
  return <section className="section products-section" id="products" aria-labelledby="products-title"><div className="container">{showHeading && <div className="section-heading split-heading"><div><p className="section-kicker">PRODUCT CENTER</p><h2 id="products-title">产品中心</h2></div><p>八款正式软件可分别购买、独立使用。每款产品都明确展示输入、处理、输出，以及可直接下载的公开文件。</p></div>}<div className="products-grid">{PRODUCTS.map((product) => <ProductCard key={product.id} product={product} featured={product.id === 'bleed' || product.id === 'multisize-bleed'} />)}</div><div className="products-footnote"><ShieldCheck size={19} weight="duotone" aria-hidden="true" /><span>八款客户端与发布校验文件均可直接下载，并按真实发布记录展示。</span><a href="/downloads/">查看全部下载 <ArrowUpRight size={15} aria-hidden="true" /></a></div></div></section>
}
...
function ProductsIndexPage() {
  return <PageShell><main className="light-page"><section className="page-intro"><div className="container"><Breadcrumbs items={[{ label: '产品中心' }]} /><p className="section-kicker">PRODUCT CENTER</p><h1>产品中心</h1><p>八款正式软件可分别购买、独立使用；每款产品的公开文件均提供直接下载入口。</p></div></section><ProductsSection showHeading={false} /><TrustSection /><HomeCTA /></main></PageShell>
}
