...
function Footer() {
  return (
    <footer className="site-footer" id="contact">
      ...
      <div className="container footer-bottom"><span>© {new Date().getFullYear()} 方寸有序工作室</span><span>本地优先 · 可复核输出 · 离线授权</span></div>
    </footer>
  )
}
...
function ProductCard({ product, featured = false }) {
  ...
  return <article className={`product-card ${featured ? 'product-card-featured' : ''}`}><div className="product-card-topline"><span className="product-eyebrow">{product.eyebrow}</span><span className={`status-badge status-${product.status.effectiveStatus}`}><span className="status-dot" />{product.status.label}</span></div><div className="product-card-icon"><GearSix size={26} weight="duotone" aria-hidden="true" /></div><h3>{product.name}</h3><p className="product-statement">{product.statement}</p><div className="product-outcome"><CheckCircle size={18} weight="duotone" aria-hidden="true" />{product.outcome}</div><div className="product-card-footer"><div><strong>{product.price.public ? product.price.display : '预约体验'}</strong><small>{productHasTrial ? product.trial.display : product.status.effectiveStatus === 'appointment' ? '公开安装包与正式价格未开放' : '下载状态以发布记录为准'}</small></div><LinkButton href={actionHref} variant={featured ? 'primary' : 'outline'}>{action.label}</LinkButton></div></article>
}
